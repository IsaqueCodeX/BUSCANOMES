...
const NameCard: React.FC<NameCardProps> = ({ data, onSelect, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <GlowingCard 
      className="flex-shrink-0 w-full max-w-[320px] h-[460px] md:h-[480px] shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.1)] hover:-translate-y-2 cursor-pointer border border-slate-100 rounded-[2rem] md:rounded-[2.5rem]"
      color={genderTheme.hex}
      onClick={() => onSelect(data)}
    >
      <div className="flex flex-col h-full">
        {/* Premium Typographic Header */}
        <div className={`h-36 md:h-44 relative flex items-center justify-center overflow-hidden flex-shrink-0 ${genderTheme.bg}`}>
          ...
          <div className="space-y-4 md:space-y-6">
            <div className="flex justify-between items-start">
              ...
              <div className="flex gap-1.5 md:gap-2">
                <button 
                  onClick={handleShare}
                  className="p-2 md:p-2.5 rounded-xl bg-slate-50 text-slate-300 hover:text-blue-500 transition-all active:scale-90"
                  title="Compartilhar"
                >
                ...
                <button 
                  ...
                  className={`p-2 md:p-2.5 rounded-xl transition-all active:scale-90 ${isFavorite ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-300 hover:text-rose-400'}`}
            ...
            <div className="space-y-3 md:space-y-4 overflow-y-auto no-scrollbar max-h-32 md:max-h-36 pr-2">
              <div>
                <p className="text-[8px] md:text-[9px] uppercase font-black text-slate-300 tracking-[0.2em] mb-1">O Significado</p>
                <p className="text-slate-700 text-xs md:text-[13px] leading-relaxed font-semibold italic">"{data.meaning}"</p>
              </div>
